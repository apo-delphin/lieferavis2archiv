...
# Konfiguration
SOURCE_PATH="/media/ASERVER/Lieferavis_PHO"  # Verzeichnis mit den entfernten Dateien
PAPERLESS_CONSUMPTION_DIR="/home/delphin/nextcloud/10_Ablagen/Archiv"  # Zielverzeichnis
PAPERLESS_CONSUMPTION_DIR_RECURSIVE="$PAPERLESS_CONSUMPTION_DIR/PHÖNIX APOTHEKE"
#PAPERLESS_CONSUMPTION_DIR_RECURSIVE="$HOME/test"
TRACKING_FILE="/home/delphin/robots/lieferavis2archiv/processed_files_PHO.txt"  # Datei zur Verfolgung der verarbeiteten Dateien

echo "robot.lieferavis2archiv.pho"
echo "LESE LIEFERSCHEINE VON PHÖNIXAPOTHEKE EIN."
echo "=========================================="
    ...
    if [ -f "$file" ]; then
        ...
        if ! grep -q "^$filename$" "$TRACKING_FILE"; then
            ...
            if cp "$file" "$PAPERLESS_CONSUMPTION_DIR_RECURSIVE/"; then
                ...
                #Nur für debugging
		echo "Datei $file wurde erfolgreich nach $PAPERLESS_CONSUMPTION_DIR_RECURSIVE kopiert."
...
echo "OK: Phönix-Uebertragung von Lieferscheinen erfolgreich abgeschlossen."
